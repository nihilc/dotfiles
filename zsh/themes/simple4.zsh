...
theme_precmd () {
  if [[ -z $(git status -s 2> /dev/null) ]]; then
    ...
    zstyle ':vcs_info:git:*' formats '%F{#1a1b26}%K{01} שׂ %b ✘ %k%f%F{01}%K{#1a1b26}%k%f'
  fi
  vcs_info
}
...
# ╭ ╰ ─    
ZSH_THEME_NL=$'\n'
ZSH_THEME_USER='%F{05}%f%F{#1a1b26}%K{05}  %n %k%f%F{05}%f'
ZSH_THEME_DIR='%F{#1a1b26}%K{04}  %3~ %k%f%F{04}%f'
ZSH_THEME_TIME='%F{#1a1b26}%K{00}%f%k%F{07}%K{00}  %T %f%k%F{00}%f'
